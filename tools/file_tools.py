    ...
    try:
        ...
        with ExitStack() as _locks:
            ...
            if mode == "replace":
                ...
                # Let an editor-backed ACP filesystem apply the original patch
                # with the already-resolved path map. If ACP cannot handle it,
                # preserve upstream's host-aware rewrite for the local fallback
                # without leaking host paths into remote/container backends.
                result = (
                    acp_filesystem.patch_v4a(patch, _path_to_resolved)
                    if acp_filesystem
                    else None
                )
                if result is None:
