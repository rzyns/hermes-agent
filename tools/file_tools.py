...
except Exception:  # pragma: no cover - ACP extra may be unavailable
...
def read_file_tool(path: str, offset: int = 1, limit: int = 2000, task_id: str = "default") -> str:
    ...
    try:
        ...
        # Prefer the ACP editor filesystem when available so unsaved buffers
        # are visible. The adapter returns None when the capability is absent
        # (or when a local-workspace resource miss should fall back to disk).
...
def write_file_tool(path: str, content: str, task_id: str = "default",
    ...
    try:
        ...
        with ExitStack() as _lock:
            ...
            write_target = _resolved or path
            result = acp_filesystem.write_text_file(write_target, content) if acp_filesystem else None
            if result is None:
                result = _get_file_ops(task_id).write_file(write_target, content)
            result_dict = result.to_dict()
            if warnings:
            ...
            if mode == "replace":
                ...
                if result is None:
                    result = file_ops.patch_v4a(
                        _rewrite_v4a_patch_paths_for_host(patch, _path_to_resolved, file_ops)
                    )
            else:
...
def search_tool(pattern: str, target: str = "content", path: str = ".",
    ...
    try:
        # Local search cannot see an ACP workspace hosted by the editor on a
        # remote machine. Fail loudly rather than returning misleading empty
        # results from a same-named local path.
        if acp_filesystem is not None and acp_filesystem.acp_remote_workspace_active():
            return json.dumps({
                "error": (
                    ...
                    "Use terminal instead; it runs in the editor workspace."
                ),
                "pattern": pattern,
            }, ensure_ascii=False)
