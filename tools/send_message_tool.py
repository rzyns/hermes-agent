...
_BLUEBUBBLES_E164_RE = re.compile(r"^\s*\+\d{7,15}\s*$")
...
def _resolve_tool_target(target: str, *, pass_unresolved_references: bool = False):
    ...
    # Preserve adapter-native targets that the channel directory cannot
    # resolve. Mattermost IDs and BlueBubbles E.164 recipients must never fall
    # through to the configured home channel.
    if platform_name == "bluebubbles" and _BLUEBUBBLES_E164_RE.fullmatch(target_ref):
        return platform_name, target_ref.strip(), None, None
    if platform_name == "mattermost" and (match := _MATTERMOST_TARGET_RE.fullmatch(target_ref)):
        return platform_name, match.group(1), match.group(2), None
