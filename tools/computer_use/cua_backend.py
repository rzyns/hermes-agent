...
from tools.computer_use.cua_backend_parse import _action_result_from, _positive_int
...
class CuaDriverBackend(_CaptureMixin, _InputMixin, ComputerUseBackend):
    ...
    def focus_app(self, app: str, raise_window: bool = False, pid: Optional[int] = None,
                  window_id: Optional[int] = None) -> ActionResult:
        """Select an app through the capture mixin, or bypass discovery with an exact window pair."""
        if pid is None and window_id is None:
            return super().focus_app(app, raise_window)
        ...
        if target_pid is None or target_window_id is None:
            ...
            return ActionResult(ok=False, action="focus_app",
                                message="focus_app requires positive integer pid and window_id when targeting directly.")
        self._set_active_target({"pid": target_pid, "window_id": target_window_id})
        if app:
            self._last_app = app
        if not raise_window:
            return ActionResult(ok=True, action="focus_app",
                                message=f"Targeted pid {target_pid}, window {target_window_id} without raising window.")
        if not self._session._has_tool("bring_to_front"):
            return ActionResult(ok=True, action="focus_app", code="bring_to_front_unsupported",
                                message=(f"Targeted pid {target_pid}, window {target_window_id} without raising window; "
