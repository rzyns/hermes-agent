...
def _apps_from_tool_result(out: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    data = out.get("data")
    apps = _first_list_field(structured, data, out, names=("apps", "_legacy_apps"))
    if apps:
        return apps
...
def _windows_from_tool_result(out: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    # Preserve the extractor's raw/minimal-envelope contract while retaining
    # the older compatibility behavior for real driver rows that carry
    # display metadata. ``_prepare_loaded_windows`` is idempotent, so callers
    # such as ``_load_windows`` can safely run it again.
    if any(
        any(key in window for key in ("title", "is_on_screen", "bounds", "z_index"))
        for window in raw_windows
    ):
        return _prepare_loaded_windows(raw_windows)
    return raw_windows
...
def _frontmost_sort_key(window: Dict[str, Any]) -> float:
    try:
        ...
        z_index = 0.0
    # CUA Driver defines larger z-index values as closer to the foreground
    # on every supported platform. Negate so ascending sort selects frontmost.
    return -z_index
...
def _ingest_windows(raw_windows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for raw in raw_windows:
        ...
        if not isinstance(raw, dict):
            continue
        pid_int = _positive_int(raw.get("pid"))
        window_id_int = _positive_int(raw.get("window_id", raw.get("id")))
...
def _prepare_loaded_windows(raw_windows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """Enrich normalized windows with process metadata used for app matching."""
    if raw_windows and all("match_text" in window for window in raw_windows):
        return raw_windows
    windows = _ingest_windows(raw_windows)
    for window in windows:
        process_name = _process_name_for_pid(window["pid"])
        has_explicit_app_name = bool(window["app_name"])
        if not has_explicit_app_name and process_name:
            window["app_name"] = process_name
        match_text = (
            f"{window['app_name']} {process_name}"
            if has_explicit_app_name
            else f"{window['app_name']} {window['title']} {process_name}"
        )
        window["match_text"] = match_text.lower()
    return windows
...
class CuaDriverBackend(ComputerUseBackend):
    ...
    def _load_windows(self) -> List[Dict[str, Any]]:
        ...
        CUA Driver reports larger ``z_index`` values closer to the foreground
        on every supported platform. ``_frontmost_sort_key`` normalizes the
        value so the first row is the default capture/focus target.
        """
        out = self._call_capture_tool(
            "list_windows",
            {"on_screen_only": True, "session": self._session_id},
        )
        windows = _prepare_loaded_windows(_windows_from_tool_result(out))
        ...
        if cli_out.get("isError") is True:
            ...
            self._clear_active_target()
            return []
        windows = _prepare_loaded_windows(_windows_from_tool_result(cli_out))
        windows.sort(key=_frontmost_sort_key)
        return windows
    ...
    def _match_windows_for_app(
        ...
        # Some X11 backends expose a title but no app name. ``_ingest_windows``
        # may fill that missing name from process metadata, so use its
        # constrained ``match_text`` marker rather than checking the normalized
        # ``app_name`` for emptiness. Explicit app-name rows deliberately omit
        # their title from ``match_text``; a localized title therefore cannot
        # override a real app identity.
        return [
            w for w in windows
            if app_lower in str(w.get("title", "")).lower()
            and app_lower in str(w.get("match_text", "")).lower()
        ]
    ...
    def capture(
        ...
        if target.get("off_screen"):
            return self._failed_capture(mode, "<no on-screen window found>")
    ...
    def list_apps(self) -> List[Dict[str, Any]]:
        ...
        if apps:
            return apps
        apps = _apps_from_windows(_windows_from_tool_result(out))
        if apps:
            return apps
        data = out.get("data")
    ...
    def list_windows(self) -> List[Dict[str, Any]]:
        return [
            {key: value for key, value in window.items() if key != "match_text"}
            for window in self._load_windows()
        ]
    ...
    def focus_app(
        ...
        # Use the same positive-integer validation as capture() so bogus
        # identifiers are rejected before we record them as active state.
        supplied_pair = pid is not None or window_id is not None
        if supplied_pair and (pid is None or window_id is None):
            self._clear_active_target()
            return ActionResult(
                ok=False,
                action="focus_app",
                message="focus_app requires both pid and window_id when targeting directly.",
            )
        if pid is not None and window_id is not None:
            ...
            if target_pid is None or target_window_id is None:
                ...
                return ActionResult(
                    ...
                    message="focus_app requires positive integer pid and window_id.",
                )
            ...
            if raise_window:
                if not self._session._has_tool("bring_to_front"):
                    return ActionResult(
                        ...
                        code="bring_to_front_unsupported",
                        message=(
                            f"Targeted pid {target_pid}, window {target_window_id} "
                            "without raising window; the connected cua-driver does not "
                            "advertise the standalone bring_to_front tool."
                        ),
                    )
                focused = self.bring_to_front(
                    pid=target_pid,
            ...
            if raise_window:
                if not self._session._has_tool("bring_to_front"):
                    return ActionResult(
                        ...
                        code="bring_to_front_unsupported",
                        message=(
                            f"Targeted {target['app_name'] or app} "
                            f"(pid {target['pid']}, window {target['window_id']}) "
                            "without raising window; the connected cua-driver does not "
                            "advertise the standalone bring_to_front tool."
                        ),
                    )
