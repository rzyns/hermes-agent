...
class ComputerUseBackend(ABC):
    ...
    def focus_app(self, app: str, raise_window: bool = False, pid: Optional[int] = None,
        ...
        """Route input to an app or an exact caller-identified window.

        Supplying both ``pid`` and ``window_id`` bypasses app-name discovery,
        matching the exact-target escape hatch on ``capture``.
        """
