...
class _NoopBackend(ComputerUseBackend):  # pragma: no cover
    ...
    def focus_app(self, app: str, raise_window: bool = False, pid: Optional[int] = None,
        ...
        self.calls.append(("focus_app", {"app": app, "raise_window": raise_window,
...
def _do_focus(backend, action, args, **_):
    exact = any(args.get(k) is not None for k in ("pid", "window_id"))
    if not args.get("app") and not exact:
        return json.dumps({"error": "focus_app requires `app` (or both `pid` and `window_id`)"})
    return backend.focus_app(args.get("app") or "", raise_window=bool(args.get("raise_window")),
                             **{k: args[k] for k in ("pid", "window_id") if args.get(k) is not None})
...
_ACTIONS: Dict[str, _ActionSpec] = {
    ...
    "focus_app": _ActionSpec(_do_focus, destructive=True,
