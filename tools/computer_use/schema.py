...
_PROPERTIES: Dict[str, Any] = {
    ...
    "pid": {
        "type": "integer",
        "description": (
            "Optional exact process target for action='capture' or 'focus_app'. Pair with window_id "
            "to bypass app/window discovery, including for X11 apps and Windows host apps controlled from WSL."
        ),
    },
    "window_id": {
        "type": "integer",
        "description": (
            "Optional exact native window target for action='capture' or 'focus_app'. Pair with pid "
            "when an external driver or OS lookup has already identified the window."
        ),
    },
    "element": {
        "type": "integer",
...
COMPUTER_USE_SCHEMA: Dict[str, Any] = {
    ...
    "description": (
        ...
        "user's task. If discovery cannot find a known window, pass pid + window_id together to "
        "capture or focus_app. If it consistently fails (empty captures, clicks not landing), have the user "
