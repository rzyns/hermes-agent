...
COMPUTER_USE_SCHEMA: Dict[str, Any] = {
    ...
    "description": (
        ...
        "cannot discover a known window, pass `pid` + `window_id` together "
        "to target it directly without `list_apps`/`list_windows`. If it "
        ...
        "properties": {
            ...
            "pid": {
                ...
                "description": (
                    ...
                    "with window_id to bypass discovery, including when an "
                    "X11 app or a Windows host app controlled from WSL cannot "
                    "be resolved by list_windows."
                ),
            },
            "window_id": {
                "type": "integer",
                "description": (
                    ...
                    "(for example, an HWND on Windows or CGWindowID on macOS). "
                    "Pair with pid when an external driver or OS lookup has "
                    "already identified the window."
                ),
            },
