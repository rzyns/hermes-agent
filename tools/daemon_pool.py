...
class DaemonThreadPoolExecutor(ThreadPoolExecutor):
    ...
    def _adjust_thread_count(self) -> None:
        ...
        # CPython 3.14 refactored worker construction to use a
        # ``_create_worker_context`` factory instead of passing initializer
        # and initargs directly to ``_worker``. Detect and use the new API
        # while keeping the old path for earlier interpreters.
        ...
        if num_threads < self._max_workers:
            ...
            # Python 3.14 stores the worker context factory on the executor.
