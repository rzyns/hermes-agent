...
def _apply_agent_section(agent, _agent_cfg):
    ...
    # Kanban prompt variants are session-static. Defaults retain the upstream v1 block.
    if agent._kanban_worker_guidance:
        from agent.prompt_builder import KANBAN_GUIDANCE_V2, KANBAN_PROGRESS_GUIDANCE
        if bool(_agent_section.get("kanban_guidance_v2", False)):
            agent._kanban_worker_guidance = KANBAN_GUIDANCE_V2
        if bool(_agent_section.get("kanban_progress_enabled", False)):
