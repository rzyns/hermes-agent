...
def run_conversation(
    ...
    try:
        ...
        if agent.api_mode == "codex_app_server":
            ...
            if isinstance(_codex_result, dict):
                final_response = _codex_result.get("final_response")
                failed = bool(_codex_result.get("failed") or _codex_result.get("error"))
        ...
        while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
            ...
            while retry_count < max_retries:
                ...
                try:
                    ...
                    # ── Auth failure: fail over once after refresh fails ───
                    # Provider OAuth/API-key errors that survive the credential
                    # refresh path should not loop on the same dead credential
                    # every turn. Escalate to the fallback chain here, mirroring
                    # the rate-limit/billing failover above. When no fallback is
                    # configured (or the chain is exhausted), fall through to the
                    # existing terminal handling + provider-specific guidance.
                    if (
                        classified.is_auth
        ...
        # Safety net for early-return paths that bypass finalize_turn().
        # Normal finalize_turn() emits this hook first; the helper is idempotent.
        try:
            ...
            logger.debug("session-end safety net failed", exc_info=True)
