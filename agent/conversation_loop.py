...
def _ollama_context_limit_error_for_tools(
    agent: Any, request_tokens: int, tools_for_request: Optional[List[dict]],
) -> Optional[str]:
    """Return a user-facing error when Ollama is loaded with too little context."""
    runtime_ctx = getattr(agent, "_ollama_num_ctx", None)
    if (
        not tools_for_request
    ...
    logger.warning(
        ...
        request_tokens, len(tools_for_request or []),
...
def _ollama_context_limit_error(agent: Any, request_tokens: int) -> Optional[str]:
    """Use the request-local selected schemas when available."""
    return _ollama_context_limit_error_for_tools(
        agent, request_tokens,
        getattr(agent, "_request_tools_for_api", getattr(agent, "tools", None)),
    )
