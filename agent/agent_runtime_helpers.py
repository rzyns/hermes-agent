...
def create_openai_client(agent, client_kwargs: dict, *, reason: str, shared: bool) -> Any:
    ...
    # Backward-compatible factory registry used by existing out-of-tree external-process
    # providers. New providers should override ProviderProfile.create_client(), but the
    # registered factory remains a supported local-fork seam during that migration.
    from providers import (
        ExternalProviderClientUnavailableError,
        get_provider_client_factory,
        resolve_provider_profile,
    )
    marker_url = str(client_kwargs.get("base_url", "") or "")
    external_profile = resolve_provider_profile(agent.provider, base_url=marker_url)
    client_factory = get_provider_client_factory(agent.provider, base_url=marker_url)
    if client_factory is not None:
        client = client_factory(**client_kwargs)
        _ra().logger.info(
            "%s provider client created (%s, shared=%s) %s",
            external_profile.name if external_profile is not None else agent.provider,
