...
def create_openai_client(agent, client_kwargs: dict, *, reason: str, shared: bool) -> Any:
    ...
    # Backward compatibility for provider plugins that registered a standalone
    # factory before ProviderProfile.create_client() became the canonical seam.
    # Keep this behind feature detection so the runtime also works with a
    # registry that has completed the newer API migration.
    try:
        import providers as _providers

        resolve_profile: Any = getattr(
            _providers, "resolve_provider_profile", None
        )
        get_factory: Any = getattr(
            _providers, "get_provider_client_factory", None
        )
        if callable(resolve_profile) and callable(get_factory):
            base_url = str(client_kwargs.get("base_url", "") or "")
            profile: Any = resolve_profile(agent.provider, base_url=base_url)
            client_factory: Any = get_factory(agent.provider, base_url=base_url)
            if client_factory is not None:
                client = client_factory(**client_kwargs)
                _ra().logger.info(
                    "%s provider client created from legacy factory "
                    "(%s, shared=%s) %s",
