...
def _coerce_provider_warning(raw: Any) -> dict[str, Any] | None:
...
def _collect_current_turn_provider_warnings(messages: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Return provider warnings emitted by assistant messages in this turn."""
    ...
    for msg in reversed(messages or []):
        ...
        if msg.get("role") != "assistant":
            continue
        raw_warnings = msg.get("provider_warnings")
        if not raw_warnings:
            continue
        ...
        for raw in raw_items:
            warning = _coerce_provider_warning(raw)
            if not warning:
                continue
            key = (
                warning.get("kind"),
                warning.get("original_model"),
                warning.get("fallback_model"),
                warning.get("trigger"),
                warning.get("api_refusal_category"),
            )
            if key in seen:
                continue
            seen.add(key)
...
def _format_provider_warning_footer(warnings: list[dict[str, Any]]) -> str | None:
    if not warnings:
        return None

    lines = []
    for warning in warnings:
        kind = warning.get("kind") or "provider_warning"
        original = warning.get("original_model")
        fallback = warning.get("fallback_model")
        trigger = warning.get("trigger")
        category = warning.get("api_refusal_category")
        message = warning.get("message")

        if kind == "model_refusal_fallback":
            if original and fallback:
                text = f"Claude Code model fallback: {original} → {fallback}"
            elif fallback:
                text = f"Claude Code model fallback to {fallback}"
            else:
                text = "Claude Code model fallback occurred"
        else:
            text = str(kind).replace("_", " ")
