...
from agent.delegation_context import child_env_lookup, is_dispatcher_owned_worker_context
...
def _record_kanban_budget_exhausted(
    agent, kanban_task: str, api_call_count: int, max_iterations: int,
    messages: list, turn_exit_reason: str, logger: logging.Logger,
) -> None:
    ...
    try:
        ...
        from hermes_cli.observability.budget_telemetry import build_overhead_envelope
        try:
            expected_run_id = int(child_env_lookup("HERMES_KANBAN_RUN_ID") or "")
        except (TypeError, ValueError):
            expected_run_id = None
        overhead_envelope = build_overhead_envelope(
            agent, api_call_count, messages=messages, turn_exit_reason=turn_exit_reason,
        )
        _conn = _kbc.connect()
        try:
...
def _stage_kanban_terminal_telemetry(
    agent, api_call_count: int, messages: list, turn_exit_reason: str, logger: logging.Logger
) -> None:
    """Stage one overhead envelope on every dispatcher-owned terminal run."""
    kanban_task = child_env_lookup("HERMES_KANBAN_TASK")
    if not kanban_task or not is_dispatcher_owned_worker_context():
        return
    try:
        from hermes_cli import kanban_db as _kb
        from hermes_cli import kanban_db_connect as _kbc
        ...
        envelope = build_overhead_envelope(
            agent, api_call_count, messages=messages, turn_exit_reason=turn_exit_reason,
        )
        conn = _kbc.connect()
        try:
            with _kb.write_txn(conn):
                _kb._stage_run_telemetry(conn, kanban_task, envelope)
        finally:
            with suppress(Exception):
                conn.close()
    except Exception:
        logger.warning("Failed to stage Kanban telemetry for task %s", kanban_task, exc_info=True)
...
def notify_session_end_once(
    agent, *, effective_task_id=None, turn_id=None, completed=False, failed=False,
    interrupted=False, turn_exit_reason="unknown",
) -> bool:
    """Emit the per-turn plugin end hook once, including early-return paths."""
    from agent.conversation_loop import logger

    session_id = getattr(agent, "session_id", None) or ""
    task = effective_task_id or getattr(agent, "_current_task_id", "") or ""
    turn = turn_id or getattr(agent, "_current_turn_id", "") or ""
    key = (session_id, task, turn)
    seen = getattr(agent, "_session_end_hook_emitted_turns", None)
    if seen is None:
        seen = set()
        with suppress(Exception):
            agent._session_end_hook_emitted_turns = seen
    if key in seen:
        return False
    seen.add(key)
    _invoke_hook_safely(
...
def _resolve_budget_fallback(
    ...
    if _kanban_task:
        _record_kanban_budget_exhausted(
            agent, _kanban_task, api_call_count, agent.max_iterations,
            messages, str(_turn_exit_reason), logger,
        )
...
def finalize_turn(
    ...
    _stage_kanban_terminal_telemetry(
        agent, api_call_count, messages, str(_turn_exit_reason), logger,
    )
