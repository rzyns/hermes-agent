...
KANBAN_GUIDANCE = (
    ...
    "file operations. Don't modify files outside it unless the task explicitly "
    "asks. If it is a git repo/worktree and you changed code, docs, tests, or "
    "review artifacts, commit by default before handoff: run tests or explain "
    "why not, inspect `git status`, stage only your intended files, and create a "
    "local commit. Do not commit secrets, bulky generated junk, mixed/unowned "
    "dirt, failed/uncertain work, or work where the task says no commit; report "
    "`no_commit_reason` instead. Never push/merge/deploy unless explicitly "
    "authorized.\n"
    "3. **Heartbeat on long operations.** Call `kanban_heartbeat(note=...)` "
    "every few minutes during long subprocesses. If your task may run longer "
    "than 1 hour, you MUST heartbeat at least hourly or the dispatcher may "
    "reclaim the task after `kanban.dispatch_stale_timeout_seconds` (default "
    "4 hours).\n"
    "4. **Block on genuine ambiguity.** If you need a human decision you cannot "
    "infer (missing credentials, UX choice, paywalled source, peer output you "
    "need first), call `kanban_block(reason=\"...\")` and stop. Don't guess.\n"
    "5. **Complete with structured handoff.** Call `kanban_complete(summary=..., "
    "metadata=...)`. Keep `summary` to 1–3 concrete sentences; put "
    "machine-readable facts in `metadata` (`changed_files`, `tests_run`, "
    "`commit`, or `no_commit_reason`). Task ids are board-local: cross-board "
    "refs must be `board/task_id` (for example `attention-intake/t_deadbeef`) "
    "and mirrored in metadata (`source_ref`, `created_followup`). If you create "
    "deliverables, keep them under `$HERMES_KANBAN_WORKSPACE`, pass absolute "
    "paths via `artifacts=[...]`, and include `metadata.artifact_manifest` or "
    "an `artifact-manifest.json` path. Never include secrets/tokens/raw PII. "
    "If code still needs human review, `kanban_comment` the metadata first, "
    "then `kanban_block(reason=\"review-required: <one-line summary>\")`.\n"
    ...
    "`metadata` are NOT uploaded). Files must exist at completion. For evidence-heavy "
    "tasks, put compact proof pointers/checksums in metadata or comments "
    "(`evidence_bundle`, `raw_transcript`, `claim_ledger`, `commands_run`) rather "
    "than uploading raw logs unless the evidence bundle is the user-facing deliverable.\n"
