...
KANBAN_GUIDANCE = (
    ...
    "If it is a git repo/worktree and you changed code, docs, tests, or review artifacts, commit by default before "
    "handoff after verification. If a safe commit is not possible, report `no_commit_reason` instead. Never push/merge/deploy "
...
KANBAN_PROGRESS_GUIDANCE = (
    ...
    "If you have done real work this turn but the task is not complete and you are not blocked, call "
    "`kanban_progress(summary=..., metadata=..., next_steps=[...])` to end the turn legitimately. This is a semantic "
    "checkpoint, not a lease: it counts as a turn and does not reset budgets or breakers, extend runtime, or "
    "complete/block the task. Use `kanban_heartbeat` separately for pure liveness during a long operation."
)

# Opt-in fork guidance keeps the current upstream protocol while adding the local fork's
# recovery-first posture, commit handoff, and auditable artifact-manifest requirements.
KANBAN_GUIDANCE_V2 = KANBAN_GUIDANCE.replace(
    "## Lifecycle\n\n",
    "## Your posture\n"
    "You are an empowered worker, not a tripwire. Finish the card. Recover from reversible snags before escalating; "
    "blocking is reserved for decisions or access only a human can supply. Never fabricate output to avoid a block.\n\n"
    "## Lifecycle\n\n",
).replace(
    "The workspace is yours for this run. Don't modify files outside it unless the task explicitly asks.\n",
    "Don't modify files outside it unless the task explicitly asks. If it is a git repo/worktree and you changed "
    "code, docs, tests, or review artifacts, commit by default before handoff: verify the work, inspect `git status`, "
    "stage only intended files, and create a local commit. Do not commit secrets, generated junk, mixed/unowned dirt, "
    "failed work, or work where the task says no commit; report `no_commit_reason` instead. Never push, merge, or deploy "
    "unless explicitly authorized.\n",
).replace(
    "3. **Heartbeat on long operations.**",
    "3. **Recover before you block.** For a transient error, missing dependency you can install, wrong flag, fixture "
    "gap, or empty result you can re-query, try up to three distinct bounded alternatives before blocking. Record what "
    "you tried. Make reversible choices inside the workspace and record them in `metadata.decisions`.\n"
    "4. **Heartbeat on long operations.**",
).replace(
    "4. **Block on genuine ambiguity.**",
    "5. **Block on genuine ambiguity.**",
).replace(
    "5. **Finish with the review model encoded by the task graph.**",
    "6. **Finish with the review model encoded by the task graph.**",
).replace(
    "6. **If follow-up work appears, create it; don't do it.**",
    "7. **If follow-up work appears, create it; don't do it.**",
).replace(
    "7. **Flag collision hotspots; don't pile on.**",
    "8. **Flag collision hotspots; don't pile on.**",
).replace(
    "paths in `metadata` are NOT uploaded). Files must exist at completion.\n",
    "paths in `metadata` are NOT uploaded). Files must exist at completion and be mirrored in "
    "`metadata.artifact_manifest` (or an `artifact-manifest.json`). Cross-board task references must use "
    "`board/task_id` and be mirrored in metadata.\n",
)
