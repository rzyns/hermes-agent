...
class CredentialPool:
    ...
    def _sync_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        try:
            ...
            if is_codex and entry.source == "manual:device_code":
                # Manual device-code rows may represent independent ChatGPT
                # accounts. Unlike the singleton mirror, they may only adopt
                # auth.json's pair when both JWTs expose the same stable
                # account identity. Missing or different identity fails closed.
    ...
    # Compatibility name retained for callers and regression tests written
    # before Codex and xAI singleton synchronization shared one implementation.
