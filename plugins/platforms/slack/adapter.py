...
class SlackAdapter(BasePlatformAdapter):
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        if slash_name in {"hermes", ""}:
            ...
            # ``/background`` was the historical canonical spelling before
            # background work split into ``/bg`` and side questions into
            # ``/btw``. Keep old ``/hermes background ...`` manifests working
            # while routing them to the current background-session command.
            subcommand_map["background"] = "/bg"
