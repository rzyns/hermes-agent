...
class MattermostAdapter(BasePlatformAdapter):
    ...
    async def _api_delete(self, path: str) -> bool:
        """DELETE an API resource whose successful response may have no JSON body."""
        import aiohttp
    ...
    async def send_typing(self, chat_id: str, metadata: _Metadata = None) -> None:
        ...
        if thread_id := (metadata or {}).get("thread_id"):
            payload["parent_id"] = thread_id
        await self._api_post(f"users/{self._bot_user_id}/typing", payload)

    def _reactions_enabled(self) -> bool:
        raw = self.config.extra.get("reactions") if self.config.extra else None
        if raw is None:
            raw = _get_scoped_secret("MATTERMOST_REACTIONS", "true")
    ...
    async def _add_reaction(self, post_id: str, emoji_name: str) -> bool:
        ...
        data = await self._api_post(
            "reactions",
            {"user_id": self._bot_user_id, "post_id": post_id, "emoji_name": emoji_name},
        )
    ...
    async def on_processing_start(self, event: MessageEvent) -> None:
        if self._reactions_enabled() and event.message_id:
...
_YAML_BRIDGE = (  # (yaml key, env var, yaml value → env string); allowed_channels is a whitelist
    ...
    ("reactions", "MATTERMOST_REACTIONS", lambda v: str(v).lower()),
