...
class HonchoMemoryProvider(DialecticMixin, MemoryProvider):
    ...
    def _writes_enabled(self) -> bool:
        ...
        return (
            not self._cron_skipped
            and not self._shutdown_event.is_set()
            and getattr(self._config, "save_messages", True)
        )
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        """Rebind cached Honcho state after Hermes changes session identity."""
        if self._cron_skipped or self._shutdown_event.is_set() or not new_session_id:
            return
    ...
    def on_memory_write(
        ...
        if not self._writes_enabled() or not self._ready_or_kick_init():
            return
        session_key = self._session_key
        self._memwrite_thread = self._spawn_write(lambda: self._manager.create_conclusion(session_key, content),
    ...
    def _close_honcho_client(self) -> None:
        """Best-effort close of sync/async SDK transports to unblock shutdown."""
        try:
