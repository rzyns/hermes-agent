...
def _behavior_fields(look: _HostLookup, explicitly_configured: bool) -> dict[str, Any]:
    ...
    raw_noise_filters = look.host.get("messageNoiseFilters", look.raw.get("messageNoiseFilters", []))
    if isinstance(raw_noise_filters, str):
        raw_noise_filters = [raw_noise_filters]
    message_noise_filters = []
    if isinstance(raw_noise_filters, list):
        for item in raw_noise_filters:
            value = (item.get("pattern") or item.get("regex") or item.get("value")) if isinstance(item, dict) else item
            text = str(value).strip() if value is not None else ""
            if text:
                message_noise_filters.append(text)
    return {
        ...
        "message_noise_filters": message_noise_filters,
...
class HonchoClientConfig:
    ...
    def resolve_session_name_candidates(
        ...
        """Ordered diagnostic candidates using the runtime resolver's precedence."""
        import re

        def slug(text: str | None) -> str | None:
            sanitized = re.sub(r"[^a-zA-Z0-9_-]+", "-", text or "").strip("-")
        ...
        if manual := self.sessions.get(cwd):
            candidates.append(("manual_override", manual))
        if title := slug(session_title):
            candidates.append(("session_title", self._with_peer_prefix(title)))
        if repo := self._git_repo_name(cwd):
            candidates.append(("repo", self._with_peer_prefix(repo)))
        candidates.append(("directory", self._with_peer_prefix(Path(cwd).name)))
        candidates.append(("global", self.workspace_id))
        return candidates
