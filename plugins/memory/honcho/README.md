...
|----------|--------|---------------------|
| 1 | Gateway session key (WebUI, Telegram, Discord, etc.) | `"agent-main-telegram-dm-8439114563"` |
| 2 | `per-session` strategy | Hermes session ID (`20260415_a3f2b1`) |
| 3 | Manual map (`sessions` config; non-per-session/local fallback) | `"myproject-main"` |
| 4 | `/title` command (display label / legacy non-gateway remap) | `"refactor-auth"` |
...
Gateway/WebUI turns always resolve via priority 1 (per-chat isolation) when a `gateway_session_key` is present, regardless of `sessionStrategy` or display title. With `sessionStrategy: "per-session"`, the Hermes `session_id` is authoritative when no valid gateway key is present, so generated titles and stale cwd maps cannot split a live conversation. Manual maps and titles still name non-per-session/local sessions.
