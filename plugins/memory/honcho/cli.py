...
def cmd_resolve_session(args) -> None:
    """Resolve the active Honcho key and show compatibility candidates."""
    try:
    ...
    primary = hcfg.resolve_session_name(**kwargs)
    candidates = hcfg.resolve_session_name_candidates(**kwargs)
    ...
    for source, key in candidates:
        print(f"  {'*' if key == primary else ' '} {source:<19} {key}")
    print()
...
_SUBCOMMANDS = (
    ...
    ("resolve-session", "Resolve the active Honcho session key", cmd_resolve_session, (
        ("--cwd", dict(metavar="PATH", help="Working directory (default: current directory)")),
        ("--session-title", dict(metavar="TITLE", help="Hermes display title candidate")),
        ("--session-id", dict(metavar="ID", help="Raw Hermes session id candidate")),
        ("--gateway-session-key", dict(metavar="KEY", help="Canonical gateway session key")),
    )),
