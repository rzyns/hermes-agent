...
def _compile_message_noise_filters(patterns: list[str] | None) -> list[re.Pattern[str]]:
    """Compile configured transient-message filters, ignoring invalid regexes."""
...
class HonchoSessionManager(SessionAuthMixin, SessionPeersMixin, SessionContextMixin, SessionMigrationMixin):
    ...
    def _should_skip_message_for_honcho(self, content: str) -> bool:
        ...
        return not text or any(pattern.search(text) for pattern in self._message_noise_filters)
    ...
    def _flush_session(self, session: HonchoSession) -> bool:
        ...
        for message in new_messages:
            if self._should_skip_message_for_honcho(message.get("content", "")):
        ...
        def _sync_messages() -> int:
            ...
            honcho_messages = [(user_peer if m["role"] == "user" else assistant_peer).message(m["content"]) for m in send_messages]
