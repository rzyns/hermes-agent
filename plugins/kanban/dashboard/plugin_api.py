...
class CreateTaskBody(StrictPayloadBase):
    ...
    worker_max_turns: Optional[int] = Field(default=None, ge=1)
...
def create_task(payload: CreateTaskBody, board: Optional[str] = Query(None)):
    ...
    try:
        ...
        task_id = kanban_db.create_task(
            ...
            worker_max_turns=payload.worker_max_turns,
        ...
        if payload.status is not None:
            ...
            if s == "done":
                ...
                if current and current.status in ("blocked", "scheduled"):
                    ...
                    ok = reopened if reopened is not None else _set_status_direct(conn, task_id, "ready", board=board)
            elif s == "archived":
                ...
                if payload.status is not None and not payload.archive:
                    ...
                    if s == "done":
                        ...
                        if cur and cur.status in ("blocked", "scheduled"):
                            ...
                            ok = reopened if reopened is not None else _set_status_direct(conn, tid, "ready", board=board)
                            if not ok:
