...
def _initialize_run_state(self, *, store_factory) -> None:
    ...
    # Public SSE transport: bounded replay rings plus per-subscriber queues.
    # Keep the legacy queue map below for in-process consumers while the
    # producer owns the externally visible run-events contract.
    ...
    *,
    ...
    def _push(event: Dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.debug(
                "[api_server] run-events producer emit failed for %s",
                run_id,
                exc_info=True,
            )
        q = self._run_streams.get(run_id)
        if q is None or not hasattr(q, "put_nowait"):
            return
        try:
...
async def _handle_runs(
    ...
    *,
    ...
    # Preserve the replay-capable SSE contract from the pre-extraction API
    # server while retaining durable idempotency and room-scoped ownership.
    ...
    async def _run_and_close():
        try:
            ...
            def _approval_notify(approval_data: Dict[str, Any]) -> None:
                ...
                try:
                    self._run_events_producer.emit_event(
                        run_id, "approval.request", event
                    )
                except Exception:
                    logger.debug(
                        "[api_server] run-events producer approval emit failed for %s",
                        run_id,
                        exc_info=True,
                    )
                try:
...
async def _handle_run_events(
    ...
    *,
    ...
    raw_includes = (
        request.query.getall("include") if "include" in request.query else []
    )
    try:
    ...
    sub_id, replay_entries, _run_active = result
    ...
    try:
        ...
        async def _serialization_watchdog() -> None:
            interval = max(
                0.01, min(float(heartbeat_interval) / 10.0, 0.25)
            )
            while True:
        ...
        while True:
            ...
            if not producer.run_is_active(run_id):
                ...
                await _write_frame(
                    producer.format_terminal_frame(
...
async def _handle_run_approval(
    ...
    *,
    ...
    with suppress(Exception):
        self._run_events_producer.emit_event(
            run_id, "approval.responded", approval_event
        )
    q = self._run_streams.get(run_id)
    if q is not None and hasattr(q, "put_nowait"):
        try:
            q.put_nowait(approval_event)
        except Exception:
            pass
...
async def _handle_steer_run(
    ...
    *,
    ...
    with suppress(Exception):
        self._run_events_producer.emit_event(run_id, "run.steered", steer_event)
    q = self._run_streams.get(run_id)
    if q is not None and hasattr(q, "put_nowait"):
        with suppress(Exception):
            q.put_nowait(steer_event)
