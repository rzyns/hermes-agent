...
def _initialize_run_state(self, *, store_factory) -> None:
    ...
    # The producer is the public SSE transport: bounded replay plus an
    # independent queue per subscriber.  The legacy queue map remains for
    # in-process control consumers.
    self._run_events_producer = RunEventsProducer(RunEventsCapabilitiesSnapshot())
...
class _RunLaunch:
    ...
    def put_event(self, event: Optional[Dict]) -> None:
        ...
        if event is None:
            self.owner._run_events_producer.mark_run_terminal(self.run_id)
        ...
        if self.owner._run_streams.get(self.run_id) is not self.queue:
            return
        self.owner._run_events_producer.emit_event(
            self.run_id, event.get("event", "error"), event)
...
async def _handle_run_events(self, request: "web.Request", *, _api_server) -> "web.StreamResponse":
    """Stream replayable run events with bounded subscriber admission."""
    ...
    # The client may subscribe immediately after receiving the 202 response.
    for _ in range(20):
    ...
    try:
        ...
        return web.json_response(exc.error_body(), status=exc.http_status)
    admitted = producer.admit_subscriber_and_snapshot(run_id, cursor, include_categories)
    if admitted is None:
        return web.json_response(
            snapshot.overload_error(), status=503, headers={"Retry-After": "1"})
    sub_id, replay_entries, _run_active = admitted
    ...
    try:
        ...
        while True:
            frame = producer.get_subscriber_frame(run_id, sub_id, timeout=poll_interval)
            ...
            if not producer.run_is_active(run_id):
                ...
                while True:
                    frame = producer.get_subscriber_frame(run_id, sub_id, timeout=0)
                    if frame is None:
                        break
                    if not await _write_frame(frame):
                        producer.mark_write_failed(run_id, sub_id)
                ...
                await _write_frame(producer.format_terminal_frame(run_id, terminal_status, final_id))
                await _write_frame(b": stream closed\n\n")
                break
...
def _mark_run_event(self, run_id: str, name: str, **fields: Any) -> None:
    ...
    event = _run_event(run_id, name, **fields)
    with suppress(Exception):
        self._run_events_producer.emit_event(run_id, name, event)
    q = self._run_streams.get(run_id)
    if q is not None:
        with suppress(Exception):
            q.put_nowait(event)
...
def _sweep_orphaned_runs_once(self, now: Optional[float] = None) -> None:
    ...
    if now is None:
        now = time.time()
    for run_id in self._run_events_producer.sweep_expired_runs(now):
