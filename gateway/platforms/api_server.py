...
from gateway.raw_provider import (
    RAW_MODEL_PREFIX as _RAW_MODEL_PREFIX,
...
def _parse_raw_model_name(model_name: Any) -> Optional[Dict[str, str]]:
    """Parse ``raw/<provider>/<model>`` without changing ordinary model routing."""
    if not isinstance(model_name, str) or not model_name.startswith(_RAW_MODEL_PREFIX):
        return None
    remainder = model_name[len(_RAW_MODEL_PREFIX):]
    provider, separator, model = remainder.partition("/")
    provider, model = provider.strip(), model.strip()
    if not separator or not provider or not model:
        raise ValueError("Raw model names must use raw/<provider>/<model>")
...
class APIServerAdapter(OpenAICompatRoutesMixin, BasePlatformAdapter):
    ...
    # Admission-gated OpenAI-compatible entry points (core bodies live in the
    # extracted mixin; the raw-provider prefix is intercepted here).
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        try:
            ...
            return web.json_response(
                _openai_error(str(exc), code="raw_model_invalid", param="model"), status=400)
        if raw_route is not None:
            origin = request.headers.get("Origin", "")
            cors_headers = self._cors_headers_for_origin(origin) if origin else None
            return await _handle_raw_chat_completions_base(
                request, body, raw_route, cors_headers=cors_headers)
        return await OpenAICompatRoutesMixin._handle_chat_completions(self, request)
    ...
    async def _handle_capabilities(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            ...
            "raw_model_proxy": {
                "enabled": True, "model_prefix": _RAW_MODEL_PREFIX,
                "chat_completions": True, "tool_execution": "client",
                "model_format": "raw/<provider>/<model>",
                "examples": ["raw/openai-codex/gpt-5.5"],
                "description": (
                    "Requests whose model begins with raw/ bypass the server-side Hermes AIAgent "
                    "and leave tool execution to the client.")},
            "features": {
    ...
    def _session_response(session: Dict[str, Any]) -> Dict[str, Any]:
        ...
        safe_keys = (
            "id", "source", "user_id", "model", "title", "title_source", "title_updated_at",
            "title_revision_count", "started_at", "ended_at", "end_reason",
