...
describe('Hermes REST helpers', () => {
  ...
  it('passes a multi-source allowlist to the all-profile session list', async () => {
    await listAllProfileSessions(50, 1, 'exclude', 'recent', 'all', { sources: ['cli', 'tui', 'webui'] })

    expect(api).toHaveBeenCalledWith(
      expect.objectContaining({
        path: '/api/profiles/sessions?limit=50&offset=0&min_messages=1&archived=exclude&order=recent&profile=all&sources=cli%2Ctui%2Cwebui',
        timeoutMs: 60_000
      })
    )
  })
  ...
  it('passes a multi-source allowlist to the batched sidebar recents slice', async () => {
    api.mockResolvedValue({ recents: { sessions: [] }, cron: { sessions: [] }, messaging: { sessions: [] } })
    ...
    expect(api).toHaveBeenCalledWith(
      expect.objectContaining({
        path:
          '/api/profiles/sessions/sidebar?recents_profile=all&recents_limit=20&cron_limit=50' +
          '&messaging_limit=100&recents_sources=cli%2Ctui%2Cwebui',
        timeoutMs: 60_000
      })
    )
  })
