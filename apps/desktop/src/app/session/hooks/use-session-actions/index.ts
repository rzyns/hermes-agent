...
interface DesktopSessionCreateParamsOptions {
  profile?: null | string
}

async function desktopSessionCreateParams(
  cwd: string,
  target: AgentProfileRoute | DesktopSessionCreateParamsOptions | null = resolveNewChatOwnerRoute()
  ...
  if (capturedRoute) {
    await ensureGatewayAgent(capturedRoute.connectionId, profile || capturedRoute.profile)
  } else {
    ...
    async (preview: string | null = null): Promise<string | null> => {
      ...
      try {
        ...
        const requestCreatedSession = <T>(method: string, requestParams?: Record<string, unknown>): Promise<T> =>
          capturedRoute
            ? requestGatewayForAgent<T>(capturedRoute.connectionId, capturedRoute.profile, method, requestParams)
        ...
        try {
          created = await requestCreatedSession<SessionCreateResponse>('session.create', params)
    ...
    async (
      ...
      try {
        ...
        if (!createFlight) {
          createFlight = (async () => {
            if (sourceSessionId) {
              return requestBranchGateway<SessionCreateResponse>('session.branch', {
                session_id: sourceSessionId,
                ...(branchCount !== undefined ? { count: branchCount } : {})
              })
            }

            // Preserve the Desktop create policy (manual model/provider,
            // effort, fast mode and routed profile) while the upstream
            // single-flight wrapper prevents duplicate children.
            const params = await desktopSessionCreateParams(cwd || '', ownerRoute ?? { profile })
