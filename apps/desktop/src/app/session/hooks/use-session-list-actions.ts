    ...
    async (shouldPublish: () => boolean = () => true) => {
      ...
      try {
        ...
        if (
          ...
          // Preserve rows from profiles the backend could not scan, but keep both
          // carried and incoming rows inside the active source allowlist.
          ...
          setSessions(prev => {
            ...
            const incoming = dropTombstoned(
              carryForwardFailedProfileSessions(
                filteredPrevious,
                filterSidebarRows(recents.sessions ?? []),
