...
# Voice provider selections are virtual when their DEFAULT_CONFIG section no
# longer seeds a concrete provider. Keep them in the UI schema so configured
# command/plugin providers remain selectable even on a blank-slate config.
for _voice_key in ("tts.provider", "stt.provider"):
    if _voice_key not in _ordered_schema and _voice_key in _SCHEMA_OVERRIDES:
        _voice_entry = dict(_SCHEMA_OVERRIDES[_voice_key])
        _voice_entry.setdefault("category", "tts")
        _ordered_schema[_voice_key] = _voice_entry
...
def _apply_model_assignment_sync(
    ...
    if scope == "main":
        ...
        # Stored provider credentials are fallbacks only. An explicit request
        # key must win (for example while rotating a stale provider key), just
        # as an explicit base_url wins over the provider entry above. When the
        # request has no key, preserve upstream's pointer-first behavior so an
        # env-backed credential is never expanded into config.yaml (#88990).
        if not api_key:
            if _assign_key_env:
...
def _discover_dashboard_plugins() -> list:
    ...
    user_plugin_roots = [
        get_process_hermes_home() / "plugins",
        get_hermes_home() / "plugins",
    ]
    root_plugins = get_default_hermes_root() / "plugins"
    if all(
        root_plugins.resolve(strict=False) != item.resolve(strict=False)
        for item in user_plugin_roots
    ):
