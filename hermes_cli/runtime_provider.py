...
def _detect_api_mode_for_url(base_url: str) -> Optional[str]:
    ...
    # MiniMax's direct API also exposes an OpenAI-compatible /v1 endpoint.
...
def _resolve_named_custom_runtime(
    *,
    ...
    # ``explicit_base_url`` is not the only intentional endpoint override:
    # resolve_runtime_provider() also derives the request from model.provider
    # + model.base_url.  Preserve that configured endpoint before the managed
    # llama.cpp shortcut below; otherwise a saved LAN llama.cpp endpoint is
    # mistaken for the managed server and fails when that server is disabled.
    ...
    if not routing_base_url:
        ...
        if (
            ...
            and _config_base_url_trustworthy_for_bare_custom(
    ...
    if requested_norm in ("llamacpp", "llama.cpp", "llama-cpp") and not routing_base_url:
        try:
...
def resolve_runtime_provider(
    *,
    ...
    should_use_pool = provider not in {"openrouter", "minimax-oauth"}
