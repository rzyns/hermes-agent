...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not adapter.is_authenticated():
        ...
        return 2
    # The standalone proxy can route to plugin-registered media providers but
    # does not initialize the normal agent runtime, so discover them explicitly.
    try:
        ...
        _err(f"proxy: plugin discovery failed: {exc}")
        return 1
