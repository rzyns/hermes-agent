...
def _core_cli_symbols(instance, *names: str):
    """Resolve symbols from the root CLI module that owns ``instance``."""
    ...
    if missing:
        raise ImportError(f"Root Hermes CLI does not expose: {', '.join(missing)}")
...
class CLIAgentSetupMixin:
    ...
    def _ensure_runtime_credentials(self) -> bool:
        ...
        ChatConsole, logger = _core_cli_symbols(self, "ChatConsole", "logger")
    ...
    def _resolve_fallback_runtime(self, primary_exc):
        ...
        _cprint, logger = _core_cli_symbols(self, "_cprint", "logger")
    ...
    def _load_resumed_history_late(self) -> bool:
        ...
        ChatConsole, _DIM, _RST, _accent_hex, _cprint = _core_cli_symbols(
            self, "ChatConsole", "_DIM", "_RST", "_accent_hex", "_cprint")
    ...
    def _init_agent(self, *, model_override: str = None, runtime_override: dict = None, request_overrides: dict | None = None) -> bool:
        ...
        ChatConsole, _cprint, _prepare_deferred_agent_startup, logger = _core_cli_symbols(
            self, "ChatConsole", "_cprint", "_prepare_deferred_agent_startup", "logger")
    ...
    def _resume_history_limit_error(self, tip_only: bool = False):
        ...
        if not self._session_db:
            return None
        logger = _core_cli_symbols(self, "logger")
    ...
    def _display_resumed_history(self):
        ...
        CLI_CONFIG, _record_output_history_entry, _strip_reasoning_tags, _suspend_output_history = _core_cli_symbols(
            self, "CLI_CONFIG", "_record_output_history_entry", "_strip_reasoning_tags", "_suspend_output_history")
        from tools.ansi_strip import sanitize_display_text as _sanitize_display_text
