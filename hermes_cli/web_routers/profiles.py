...
def get_profiles_sessions(
    ...
    filters = dict(
        ...
        sources=None if source_filter else (_csv_list(sources) or None),
...
def get_profiles_sessions_sidebar(
    ...
    recents_sources: Optional[str] = None, cron_limit: int = 50,
    messaging_limit: int = 100, messaging_exclude: str = None):
    ...
    # (source, sources, exclude) per slice; ``source=cron`` is the implicit cron taxonomy.
    slice_scope = {
        "recents": (None, recents_sources_list, recents_exclude_list),
        "cron": ("cron", None, None),
        "messaging": (None, None, messaging_exclude_list),
    }
    ...
    def _slice(db, key):
        source, sources, exclude = slice_scope[key]
        ...
        return db.list_sessions_rich(
            source=source, sources=sources or None, exclude_sources=exclude or None,
            limit=cap[key], offset=0,
    ...
    for name, home in targets:
        ...
        profile_cache_key = (str(db_path), _sidebar_db_fingerprint(db_path), cap["recents"],
                             tuple(recents_sources_list), tuple(recents_exclude_list),
                             cap["cron"], cap["messaging"],
