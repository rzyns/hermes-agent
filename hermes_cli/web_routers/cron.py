...
async def list_cron_jobs(profile: str = "all"):
    return await _run_cron_dashboard_io(late_attr("_list_cron_jobs_sync"), profile)


@router.get("/api/cron/jobs/{job_id}")
async def get_cron_job(job_id: str, profile: Optional[str] = None):
    return await _run_cron_dashboard_io(late_attr("_get_cron_job_sync"), job_id, profile)


@router.get("/api/cron/jobs/{job_id}/runs")
async def list_cron_job_runs(job_id: str, profile: Optional[str] = None, limit: int = 20):
    return await _run_cron_dashboard_io(late_attr("_list_cron_job_runs_sync"), job_id, profile, limit)


@router.post("/api/cron/jobs")
async def create_cron_job(body: CronJobCreate, profile: Optional[str] = None):
    return await _run_cron_dashboard_io(late_attr("_create_cron_job_sync"), body, profile)
...
async def update_cron_job(job_id: str, body: CronJobUpdate, profile: Optional[str] = None):
    return await _run_cron_dashboard_io(late_attr("_update_cron_job_sync"), job_id, body, profile)


@router.post("/api/cron/jobs/{job_id}/pause")
async def pause_cron_job(job_id: str, profile: Optional[str] = None):
    return await _run_cron_dashboard_io(late_attr("_pause_cron_job_sync"), job_id, profile)


@router.post("/api/cron/jobs/{job_id}/resume")
async def resume_cron_job(job_id: str, profile: Optional[str] = None):
    return await _run_cron_dashboard_io(late_attr("_resume_cron_job_sync"), job_id, profile)


@router.post("/api/cron/jobs/{job_id}/trigger")
async def trigger_cron_job(job_id: str, profile: Optional[str] = None):
    return await _run_cron_dashboard_io(late_attr("_trigger_cron_job_sync"), job_id, profile)


@router.delete("/api/cron/jobs/{job_id}")
async def delete_cron_job(job_id: str, profile: Optional[str] = None):
    return await _run_cron_dashboard_io(late_attr("_delete_cron_job_sync"), job_id, profile)
...
async def cron_fire_webhook(request: Request):
    ...
    # dashboard endpoints.
    profile = await _run_cron_dashboard_io(late_attr("_find_cron_job_profile"), job_id)
    if not profile:
