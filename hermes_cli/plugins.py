...
VALID_HOOKS: Set[str] = {
    ...
    "transform_llm_output", "pre_llm_call", "select_tool_schemas", "post_llm_call",
    ...
    "pre_gateway_dispatch", "cli:update", "command:update",
...
class PluginContext:
    ...
    def register_kanban_dependency_provider(self, provider) -> None:
        """Register a deterministic external-dependency provider for Kanban."""
        ...
        logger.info("Plugin '%s' registered Kanban dependency provider: %s",
                    self.manifest.name, getattr(provider, "name", provider.__class__.__name__))
        ...
        with self._discovery_lock, _plugin_home_scope(self.home_path):
            ...
            if force:
                with suppress(Exception):
                    from hermes_cli import kanban_dependencies as kd
                    for token in self._kanban_dependency_providers:
