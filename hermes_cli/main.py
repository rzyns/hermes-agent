...
def _load_root_cli_main():
    """Load root ``cli.py`` by path so profile plugins cannot shadow it."""
...
def cmd_update(args):
    ...
    if _update_preflight_handled(args):
        return
    # Local policy plugins may deny or fully handle an update before any checkout mutation.
    try:
