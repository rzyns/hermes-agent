...
def cmd_update_maintenance(args):
    ...
    if getattr(args, "capabilities", False):
        ...
        print(json.dumps({"schema": 1, "command": "update-maintenance", "requires_fresh_process": True}))
        return
    ...
    try:
        ...
        print()
        _run_update_maintenance(args, gateway_mode=gateway_mode, source_updated=False)
    finally:
        _finalize_update_output(_update_io_state)
...
def _run_update_maintenance(
    ...
    *,
    ...
    synchronization. Built-in ``hermes update`` performs those before calling
    this helper; fork-safe update flows can call this in a fresh process after
    their own source reconciliation.
    """
    ...
    uv_result = ensure_uv()
    if isinstance(uv_result, tuple):
        uv_bin, fresh_bootstrap = uv_result
    else:
        uv_bin = str(uv_result) if uv_result else None
        fresh_bootstrap = bool(getattr(uv_result, "fresh_bootstrap", False))
    ...
    if (desktop_dir / "package.json").exists() and shutil.which("npm") and has_desktop_app:
        ...
        # Stream the build output live (long Electron builds otherwise look
        # hung). On the rare nonzero exit, retry once after waiting again for
        # the venv — this covers a still-settling rebuild window the first wait
        # didn't fully catch.
    ...
    if source_updated:
        ...
        print("✓ Source already reconciled; running maintenance")

    # Seed the model-catalog disk cache from the freshly-pulled checkout.
    # The repo ships the canonical catalog at website/static/api/model-catalog.json,
    # and `git pull` just made it current — so copy it straight over the local
    # cache instead of waiting on a network fetch (which can be bot-gated or hit
    # a Portal hiccup). Non-fatal on failure: the normal network refresh still
    # applies on the next picker open.
    try:
        from hermes_cli.model_catalog import seed_cache_from_checkout
...
def main():
    ...
    update_maintenance_parser = subparsers.add_parser(
        "update-maintenance",
        help=argparse.SUPPRESS,
        description="Run post-source-update maintenance without git source reconciliation",
    )
    update_maintenance_parser.add_argument(
        "--gateway",
        action="store_true",
        default=False,
    ...
    update_maintenance_parser.set_defaults(func=cmd_update_maintenance)
