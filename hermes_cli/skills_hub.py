...
def _looks_like_direct_github_identifier(identifier: str) -> bool:
    """True for source-qualified GitHub refs and GitHub tree/blob URLs."""
    ...
    if ident.lower().startswith(("http://", "https://")):
        ...
        return parsed.netloc.lower() == "github.com" and len(parts) >= 5 and parts[2] in {"tree", "blob"}
    known_prefixes = (
        "official/", "skills-sh/", "skills.sh/", "skils-sh/", "skils.sh/",
        "well-known:", "url:", "clawhub/", "claude-marketplace/", "lobehub/",
        "browse-sh/", "hermes-index/",
    )
    return not ident.startswith(known_prefixes) and len([part for part in ident.split("/") if part]) >= 3
...
def _resolve_local_skill(identifier: str):
    """Return metadata + bundle for a visible profile/external filesystem skill."""
    from agent.skill_utils import get_external_skills_dirs, iter_skill_index_files
    from tools.skills_hub_models import SkillBundle, SkillMeta
    from tools.skills_tool import _find_all_skills, _get_category_from_path, _parse_frontmatter, _skills_dir

    candidate = (identifier or "").strip().split("/")[-1]
    visible = {str(skill.get("name") or "") for skill in _find_all_skills(skip_disabled=True)}
    if candidate not in visible:
        return None, None

    for scan_dir in [_skills_dir(), *get_external_skills_dirs()]:
        ...
        for skill_md in iter_skill_index_files(scan_dir, "SKILL.md"):
            try:
                ...
                if not description:
                    description = next(
                        (line for line in map(str.strip, body.splitlines()) if line and not line.startswith("#")),
                        "",
                    )
                ...
                for subdir in ("references", "templates", "scripts", "assets"):
                    root = skill_md.parent / subdir
                    if not root.is_dir():
                        continue
                    for child in root.rglob("*"):
                        if child.is_file():
                            try:
                                files[child.relative_to(skill_md.parent).as_posix()] = child.read_text(encoding="utf-8")
                            except (OSError, UnicodeDecodeError):
                                pass
                meta = SkillMeta(
                    name=name, description=description, source="local", identifier=name,
                    trust_level="local", tags=tags if isinstance(tags, list) else [],
                    extra={"category": _get_category_from_path(skill_md) or "", "local_path": str(skill_md)},
                )
                return meta, SkillBundle(
                    name=name, files=files, source="local", identifier=name, trust_level="local",
                )
...
def _resolve_inspect_identifier(identifier: str, sources, console: Console):
    """Resolve a hub identifier, then visible local skills, without noisy false errors."""
    ident, meta, bundle, src = _resolve_identifier(identifier, sources, Console(quiet=True))
    if ident and meta:
        return ident, meta, bundle, src
    local_meta, local_bundle = _resolve_local_skill(identifier)
    if local_meta:
        return local_meta.identifier, local_meta, local_bundle, None
    return _resolve_identifier(identifier, sources, console)
...
def do_inspect(identifier: str, console: Optional[Console] = None) -> None:
    ...
    c = console or _console
    identifier, meta, bundle, _src = _resolve_inspect_identifier(identifier, _sources(), c)
    if not identifier:
        return
    if not meta:
...
def _render_inspect(meta, bundle, c: Console) -> None:
    """Render inspect output for hub and local filesystem skills."""
    c.print()
...
def inspect_skill(identifier: str) -> Optional[dict]:
    ...
    ident, meta, bundle, _ = _resolve_inspect_identifier(identifier, _sources(), quiet)
