...
def _sync_codex_pool_entries(
    ...
    synced. ``manual:device_code`` (``hermes auth add openai-codex``) is synced when its access_token
    equals the PREVIOUS singleton token (a legacy alias), or when it has the same decodable account
    identity as the fresh singleton. Different-account and undecodable manual entries remain
    independent. ``manual:api_key`` and any other source are never overwritten by a re-auth.
    ...
    if entries is None:
        return
    from hermes_cli.auth_oauth_grants import _oauth_identity

    # None/empty prev_at → no manual entry can be a legacy token alias.
    ...
    for entry in _codex_pool_dicts(entries):
        ...
        entry_identity = _oauth_identity(entry) if source == "manual:device_code" else None
        refresh_manual = source == "manual:device_code" and bool(
            (prev_at and entry.get("access_token") == prev_at)
            or (new_identity and entry_identity == new_identity)
        )
        if not (source == "device_code" or refresh_manual):
            continue
