...
def _run_job_impl(
    ...
    #
    # Jobs that mutate process-global runtime state (workdir and/or profile
    # jobs) are serialized outside the parallel pool.  The TERMINAL_CWD env
    # override itself is additionally guarded by _terminal_cwd_lock (acquired
    # just below): a workdir job holds it as a writer for its whole run,
    # excluding every other job, while workdir-less jobs hold it as readers and
    # stay parallel with each other.  The runtime-context gate keeps profile
    # env/HERMES_HOME mutations from overlapping non-mutating jobs; the cwd lock
    # keeps a concurrently-firing workdir-less job from observing this override
    # and running shell/file/code-exec commands in the wrong directory.  For
    # workdir-less jobs we leave TERMINAL_CWD untouched — preserves the original behaviour
...
def tick(
    ...
    *,
    ...
    try:
        ...
        # profile-loaded env). They MUST queue on the single-thread sequential
        # pool to avoid corrupting each other. Non-mutating jobs stay
        # parallel-safe, but still enter a read gate so they cannot overlap an
        # active context-mutating job; run_job's _terminal_cwd_lock also stops a
        # concurrently firing workdir-less job from observing a workdir override.
