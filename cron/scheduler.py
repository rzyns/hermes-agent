...
class _RuntimeContextIsolation:
    ...
    Profile jobs temporarily change scheduler-wide runtime state
    (``_hermes_home`` and ``os.environ``). They must not overlap with ordinary
    jobs, because ordinary jobs resolve HERMES_HOME/script paths via the same
    globals. Workdirs are task-scoped and do not require this gate; ordinary
    jobs may still run in parallel with each other.
    """
...
def _job_mutates_runtime_context(job: dict) -> bool:
    return bool((job.get("profile") or "").strip())

# Profile jobs mutate process-global runtime state and must run one at a time,
# but must NOT block the ticker thread. A persistent single-thread executor
# preserves ordering across ticks while keeping dispatch fire-and-forget.
...
def _cron_inactivity_seconds() -> float:
    ...
    Used by run_job's inactivity monitor, which maps 0 to "no limit".
    """
...
def _get_sequential_pool() -> concurrent.futures.ThreadPoolExecutor:
    ...
    A single worker guarantees profile jobs never overlap, even
    ...
    *,
    ...
    try:
        ...
        def _process_job(job: dict, *, reserved_runtime_write: bool = False) -> bool:
            """Claim and run one due job under runtime-context isolation."""
            try:
                runtime_gate = (
                ...
                with runtime_gate:
                    # Acquire the durable claim only when this worker actually
                    # starts, not while it waits in the executor queue.
                    try:
                        claimed = claim_job_for_fire(job["id"], return_job=True)
                    except TypeError as exc:
                        # Backward compatibility for extensions and test doubles
                        # that still implement the pre-return_job one-argument
                        # contract. Do not mask unrelated TypeErrors raised by the
                        # claim implementation itself.
                        if "return_job" not in str(exc):
                            raise
                    ...
                    if not claimed:
                        ...
                        return False
                    # Production CAS returns the exact persisted record with its
                    # unique owner. Bool fallback keeps older test doubles/API
                    # overrides compatible.
                    claimed_job = (
                        dict(claimed) if isinstance(claimed, dict) else dict(job)
                    )
        ...
        # Profile jobs mutate process-global HERMES_HOME/environment state and
        # MUST queue on the single-thread sequential pool. Workdir-only jobs are
        # task-scoped and stay in the normal parallel lane. Parallel jobs still
        # enter the read gate so they cannot overlap an active profile context.
        ...
        # Sequential pass for profile jobs.
        # Queued to a persistent single-thread pool so they run one at a time
        # WITHOUT blocking the ticker thread — a long profile job no
