...
class TestNormalizeSkillLookupName:
    ...
    def test_absolute_via_external_dir_symlink_uses_resolved_relative_path(self, tmp_path, monkeypatch):
        from agent.skill_utils import normalize_skill_lookup_name

        skills_dir = tmp_path / "skills"
        skills_dir.mkdir()
        external_root = tmp_path / "external"
        skill_dir = external_root / "category" / "my-skill"
        skill_dir.mkdir(parents=True)
        linked_root = tmp_path / "linked-external"
        try:
            linked_root.symlink_to(external_root)
        except OSError:
            pytest.skip("Symlinks not supported")

        monkeypatch.setattr("tools.skills_tool.SKILLS_DIR", skills_dir)
        monkeypatch.setattr("agent.skill_utils.get_external_skills_dirs", lambda: [external_root])
        assert normalize_skill_lookup_name(str(linked_root / "category" / "my-skill")) == "category/my-skill"
