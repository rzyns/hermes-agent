...
class TestBuildTitleContext:
    ...
    def test_summarizes_skill_invocation_before_building_rich_context(self):
        skill_body = "Kick off a task in a fresh isolated git worktree. " * 20
        expanded = (
            '[IMPORTANT: The user has invoked the "work" skill, indicating they want '
            "you to follow its instructions. The full skill content is loaded below.]\n\n"
        ...
        context = build_title_context([], current_user_message=expanded)

        assert "/work — fix the session title leak" in context
        assert "Kick off a task in a fresh isolated git worktree" not in context
...
class TestAutoTitleSession:
    ...
    def test_allows_retitle_when_existing_title_is_auto_generated(self):
        ...
        db.set_auto_title_if_empty.assert_called_once_with("sess-1", "Better Specific Title")
