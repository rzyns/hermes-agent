...
class TestCompactRows:
    ...
    def test_compact_projection_tracks_schema(self, db):
        ...
        prompt storage fields — including gateway/desktop fields (git_branch,
        ...
        # Hardcode the sanctioned payload/internal exclusions: if the set ever
        # widens (or the projection silently drops a column), this fails and
        # forces a conscious review of what list consumers lose. The Git metadata
        # generation is an internal compare-and-swap token, not sidebar data.
        excluded = {
            "system_prompt",
            "system_prompt_hash",
            "git_metadata_generation",
        }
        missing = live_cols - set(row) - excluded
        assert not missing, f"compact projection lost schema columns: {missing}"
        assert "system_prompt" not in row
        assert "system_prompt_hash" not in row
        assert "git_metadata_generation" not in row
