...
class TestNoOverlayFlag:
    ...
    def test_linux_wayland_keeps_overlay(self, monkeypatch):
        ...
        # This suite may run under WSL, whose real /proc/version would take the
        # intentional WSL no-overlay branch before the Wayland branch under
        # test. Pin only the kernel identity, not the host OS.
        with patch("hermes_cli.config.load_config", return_value={}), \
             patch("builtins.open", mock_open(read_data="Linux version 6.8.0-generic")):
