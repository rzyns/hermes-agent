...
class TestStartRun:
    ...
    @pytest.mark.asyncio
    async def test_approval_request_is_published_to_replayable_events(self, adapter):
        app = _create_runs_app(adapter)
        emitted = []
        original_emit = adapter._run_events_producer.emit_event

        def _capture_emit(run_id, event_name, payload):
            emitted.append((run_id, event_name, dict(payload)))
            return original_emit(run_id, event_name, payload)

        async with TestClient(TestServer(app)) as cli:
            with (
                patch.object(adapter, "_create_agent") as mock_create,
                patch.object(
                    adapter._run_events_producer,
                    "emit_event",
                ...
                def _request_approval(
                    ...
                    notify({"request_id": "approval-1", "command": "echo safe"})
        ...
        approval_events = [
            payload
            for emitted_run_id, event_name, payload in emitted
            if emitted_run_id == run_id and event_name == "approval.request"
        ]
        assert len(approval_events) == 1
        assert approval_events[0]["request_id"] == "approval-1"
