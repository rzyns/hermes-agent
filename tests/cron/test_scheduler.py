...
class TestCronContinuableSurfaceInChannel:
    ...
    def test_in_channel_seeds_shared_channel_session_flat(self):
        ...
        # The flat session row must be CREATED (this is what was missing). The
        # local dual-surface behavior also seeds the delivered message's thread.
        seeded = next(
            call.args[0]
            for call in adapter._session_store.get_or_create_session.call_args_list
            if call.args[0].thread_id is None
        )
        ...
        # Brief mirrored flat into that row (alongside the thread seed).
        flat_mirror = next(call for call in mirror_mock.call_args_list if call.kwargs.get("thread_id") is None)
        assert flat_mirror.args[0] == "slack"
        assert flat_mirror.args[1] == "C123"
        assert "Here is today's brief." in flat_mirror.args[2]
    ...
    def test_in_channel_dm_seeds_dm_session(self):
        ...
        assert any(call.kwargs.get("thread_id") is None for call in mirror_mock.call_args_list)
