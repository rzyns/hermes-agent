...
def test_local_openviking_port_is_open_detects_listener_and_closed_port():
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as listener:
        ...
        # Drain the probe connection before closing the listener.
        accepted, _address = listener.accept()
        accepted.close()

    # Socket closed: the same port eventually stops accepting connections.
    # WSL can permit one immediate post-close connect while propagating the
    # listener teardown, so bound the retry instead of asserting on that race.
