...
class TestDispatch:
    ...
    def test_focus_app_forwards_exact_pid_window_target(self, noop_backend):
        from tools.computer_use.tool import handle_computer_use

        parsed = json.loads(handle_computer_use({
            "action": "focus_app", "pid": 23502, "window_id": 58720504,
        }))

        assert parsed["ok"] is True
        focus_kw = next(c[1] for c in noop_backend.calls if c[0] == "focus_app")
        assert focus_kw == {
            "app": "", "raise_window": False, "pid": 23502, "window_id": 58720504,
        }
...
class TestFocusAppFilterNoMatch:
    ...
    def test_exact_target_bypasses_window_discovery(self):
        backend = _make_cua_backend_with_windows([])

        res = backend.focus_app("Hermes", pid=34300, window_id=27984774)

        assert res.ok is True
        assert backend._active_pid == 34300
        assert backend._active_window_id == 27984774
        assert backend._last_target == {"pid": 34300, "window_id": 27984774}
        cast(Any, backend._session.call_tool).assert_not_called()

    @pytest.mark.parametrize("pid,window_id", [(34300, None), (None, 27984774), (0, 27984774)])
    def test_invalid_exact_target_fails_closed(self, pid, window_id):
        backend = _make_cua_backend_with_windows([])
        backend._active_pid, backend._active_window_id = 1, 2

        res = backend.focus_app("Hermes", pid=pid, window_id=window_id)
...
class TestZIndexSorting:
    ...
    def test_wsl_windows_host_order_still_selects_highest_z_index(self, monkeypatch):
        """A Windows cua-driver reached from WSL keeps the normalized highest-first contract."""
        monkeypatch.setenv("HERMES_CUA_HOST_PLATFORM", "win32")
        windows = [
            {"app_name": "ApCent.exe", "pid": 19432, "window_id": 131840,
             "is_on_screen": True, "title": "APP Center", "z_index": 0},
            {"app_name": "msedge.exe", "pid": 19148, "window_id": 132576,
             "is_on_screen": True, "title": "Hermes", "z_index": 24},
        ]
        backend = _make_cua_backend_with_windows(windows)
