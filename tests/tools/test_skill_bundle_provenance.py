...
def test_github_source_rejects_symlink_in_referenced_directory(monkeypatch):
    ...
    source._tree_cache["owner/repo@<default>"] = (
        "main",
        [
...
def test_github_source_fetches_full_directory_and_records_tree_revision(monkeypatch):
    source = GitHubSource(GitHubAuth())
    ...
    monkeypatch.setattr(
        source,
        "_fetch_file_content",
        lambda _repo, path, ref=None: skill if path.endswith("SKILL.md") else None,
    )

    def _fetch_bytes(_repo, path, ref=None):
        fetched.append(path)
        return b"guide"
    ...
    assert fetched == [
        ...
        "skill/references/unreferenced.md",
    ]
    assert bundle.files["references/guide.md"] == b"guide"
    assert bundle.files["references/unreferenced.md"] == b"guide"
