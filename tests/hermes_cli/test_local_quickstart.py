...
def test_quickstart_runs_all_three_legs(client, monkeypatch, tmp_path):
    ...
    # Hardware discovery is host-dependent (notably WSL reports NVIDIA, while
    # Linux has no prebuilt CUDA archive). Keep this integration test on the
    # portable CPU install path; backend selection has its own matrix tests.
    monkeypatch.setattr(
