...
def test_protocol_violation_limit_preserves_board_scoped_self_heal(
    kanban_home, monkeypatch,
):
    """The bounded upstream retry still enters the local repair-card lane.

    ``max_retries=1`` makes the first protocol violation exhaust the bounded
    streak. The merge must preserve the caller's explicit board when the
    resulting auto-block creates a self-healing repair task.
    """
    import hermes_cli.kanban_db as _kb
    ...
    def _record_repair(_conn, task_id, **kwargs):
        calls.append((task_id, kwargs))
        return "repair-task"

    try:
        tid = kb.create_task(
            conn, title="strict repair", assignee="worker", max_retries=1,
        )
        host_prefix = _kb._claimer_id().split(":", 1)[0]
        ...
        monkeypatch.setattr(_kb, "_self_heal_config", lambda: (True, "ops"))
        monkeypatch.setattr(_kb, "create_swarm_repair_task", _record_repair)

        _kb.detect_crashed_workers(conn, board="research")
