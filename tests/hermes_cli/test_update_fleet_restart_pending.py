...
def _patch_update_deps(monkeypatch, tmp_path, run_side_effect):
    ...
    monkeypatch.setattr(
        update_cmd, "_editable_install_is_current", lambda *a, **k: True
    )
    monkeypatch.setattr(
        hermes_main, "_verify_core_dependencies_installed", lambda *a, **k: None
    )
    monkeypatch.setattr(
        hermes_main, "_verify_console_scripts_installed", lambda *a, **k: None
    )
    monkeypatch.setattr(
        hermes_main, "_restore_active_tool_dependencies", lambda *a, **k: None
    )
    monkeypatch.setattr(
        "hermes_cli.managed_uv.update_managed_uv", lambda *a, **k: None
    )
    monkeypatch.setattr(
        "hermes_cli.managed_uv.ensure_uv", lambda *a, **k: None
    )
    monkeypatch.setattr(
