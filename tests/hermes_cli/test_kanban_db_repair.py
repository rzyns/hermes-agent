...
def test_cli_repair_candidate_subcommand_keeps_candidate_workflow(
    cli_home, monkeypatch
):
    """The new bare integrity verb must not swallow existing subcommands."""
    from hermes_cli import kanban as kc

    calls: list[str] = []
    monkeypatch.setattr(
        kc,
        "_cmd_repair_candidates",
        lambda args: calls.append(args.repair_action) or 17,
    )

    rc = _run_kanban_cli(["repair", "list-candidates", "default", "--json"])
