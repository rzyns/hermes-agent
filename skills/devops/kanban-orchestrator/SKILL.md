...
Hallucination warnings appear on tasks where a worker's `kanban_complete(created_cards=[...])` claim included card ids that don't exist or weren't created by the worker's profile (the gate blocks the completion), where the free-form summary references `t_<hex>` ids that don't resolve (advisory phantom-reference scan, non-blocking), or where a bare task id exists only on another board and should be written as `board/task_id` (advisory unqualified cross-board warning). These audit events persist even after recovery actions — the trail stays for debugging.
