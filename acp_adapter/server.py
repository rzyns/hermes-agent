...
class HermesACPAgent(SlashCommandsMixin, acp.Agent):
    ...
    def _run_agent_turn(
        ...
        with contextlib.ExitStack() as stack:
            ...
            # Route file and terminal operations through the editor when the ACP
            # client advertises those capabilities. This preserves dirty-buffer
            # reads/writes and remote-workspace execution without bypassing the
            # upstream turn-binding lifecycle.
            if conn and self._client_capabilities:
                stack.enter_context(
                    acp_filesystem.use_acp_filesystem(
                        client=conn, session_id=session_id, loop=loop, cwd=state.cwd,
